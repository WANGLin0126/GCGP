...
# for dataset in  ogbn-products ; do  # Cora Citeseer Pubmed Computers Photo
#     for cond_size in 197 983 1966; do
#         for ridge in 1e-3 1e-2 1e-1 1e0 1e1; do
#             for k in 1 2 3; do
#                 for learn_A in 0 1; do
#                     for norm in 0; do
#                         # 打印输出
#                         echo ${dataset}_size_${cond_size}_ridge_${ridge}_k_${k}_learn_A_${learn_A}
#                         python main.py --dataset $dataset --cond_size $cond_size --ridge $ridge --k $k --kernel SGNK --epochs 100 --train_batch_size 800 --learn_A $learn_A --norm $norm > outputs/${dataset}_size_${cond_size}_ridge_${ridge}_k_${k}_learn_A_${learn_A}_norm_${norm}.txt
#                     done
#                 done
#             done
#         done
