...
class KernelRidgeRegression(nn.Module):
    ...
    def forward(self, G_t, G_s, y_t, y_s, A_t, A_s, Alpha, epoch=0, train=1, learnA=0, norm=0):
        if learnA:
            if train:
    ...
    # def forward(self, G_t, G_s, y_t, y_s, A_t, A_s, epoch=0, train=True, learnA=0, norm=0):
    #     if learnA:
    #         if train:
