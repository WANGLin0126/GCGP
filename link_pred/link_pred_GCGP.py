"""
using the condensed graph from GCGP method to training a link prediction model
"""
...
def load_custom_train_data(x_path='x.pt', A_path='A.pt'):
    ...
    try:
        ...
        # 移除自环边
        if args.dataset == "Citeseer" and args.cond_ratio == "0.25":
            pass
        else:
...
try:
    train_data = load_custom_train_data(
        # x_path="./saved_GCGP/Cora_x_s_SGNK_1.0_learnA_1.pt", 
        # A_path="./saved_GCGP/Cora_A_s_SGNK_1.0_learnA_1.pt"
        x_path=f"./saved_GCGP/{args.dataset}_x_s_SGNK_{args.cond_ratio}_learnA_1.pt", 
        A_path=f"./saved_GCGP/{args.dataset}_A_s_SGNK_{args.cond_ratio}_learnA_1.pt"
    )
...
def test(split='test'):
    ...
    try:
        ...
        # 前向传播
        z, _ = model(data.x, pos_edge_index)
